"""Visualization utilities for LUAD simulation outputs."""
...
COLOR_MAP = {
    0: (0.0, 0.0, 0.0),       # empty
    1: (0.95, 0.4, 0.4),      # epithelial / tumor
    2: (0.8, 0.6, 0.2),       # CAF / αSMA
    3: (0.2, 0.6, 0.9),       # CD8
    4: (0.3, 0.8, 0.9),       # CD4
    5: (0.6, 0.2, 0.8),       # Treg
    6: (0.55, 0.55, 0.55),    # Macrophage
    7: (0.4, 0.9, 0.4),       # TLS
    8: (0.9, 0.9, 0.9),       # other
}

def plot_trajectory_palette() -> dict[str, str]:
    return {
        "tumor_count": "#F2625F",
        "caf_count": "#CC9A33",
        "cd8_count": "#3498DB",
...
def plot_trajectories(timeseries: pd.DataFrame, output_dir: Path, palette: dict[str, str] | None = None) -> Path:
    fig, ax = plt.subplots(figsize=(8, 5))
    count_cols = [c for c in timeseries.columns if c.endswith("_count")]
    for column in count_cols:
        label = column.replace("_count", "")
        color = palette.get(column) if palette else None
        ax.plot(
            timeseries.index,
            timeseries[column],
            label=label,
...
def plot_state_metrics(timeseries: pd.DataFrame, output_dir: Path) -> Path:
    fig, ax = plt.subplots(figsize=(8, 4))
    for column in ["emt_high_fraction", "ecm_fraction", "tls_quality_mean"]:
        if column in timeseries:
            ax.plot(timeseries.index, timeseries[column], label=column)
    ax.set_ylim(0, 1)
    ax.set_xlabel("Tick")
    ax.set_ylabel("Fraction")
    ax.set_title("State metrics")
    ax.legend(loc="upper right", fontsize=8)
    ax.grid(True, linestyle="--", alpha=0.3)
    path = output_dir / "state_metrics.png"
    fig.tight_layout()
    fig.savefig(path, dpi=200)
    plt.close(fig)
    return path
...
def plot_tls_quality(timeseries: pd.DataFrame, output_dir: Path) -> Path:
    if "tls_quality_mean" not in timeseries or "cd8_count" not in timeseries:
        return output_dir / "tls_quality.png"
    fig, ax1 = plt.subplots(figsize=(8, 4))
    ax1.plot(timeseries.index, timeseries["tls_quality_mean"], color="tab:green", label="TLS quality")
    ax1.set_ylabel("TLS quality", color="tab:green")
    ax1.tick_params(axis='y', labelcolor="tab:green")

    ax2 = ax1.twinx()
    effector_fraction = (timeseries["cd8_count"].astype(float) /
                         (timeseries["cd8_count"] + timeseries.get("treg_count", 0.0)).replace(0, np.nan))
    ax2.plot(timeseries.index, effector_fraction, color="tab:blue", label="Effector fraction")
    ax2.set_ylabel("Effector fraction", color="tab:blue")
    ax2.tick_params(axis='y', labelcolor="tab:blue")

    ax1.set_xlabel("Tick")
    ax1.set_title("TLS quality vs effector fraction")

    path = output_dir / "tls_quality.png"
    fig.tight_layout()
    fig.savefig(path, dpi=200)
    plt.close(fig)
    return path
...
def build_legend(height: int, frame_width: int) -> np.ndarray:
    entries = [
        ("Tumor", COLOR_MAP[1]),
        ("CAF/alphaSMA", COLOR_MAP[2]),
        ("CD8 T", COLOR_MAP[3]),
        ("CD4 T", COLOR_MAP[4]),
        ("Treg", COLOR_MAP[5]),
        ("Macrophage", COLOR_MAP[6]),
        ("TLS node", COLOR_MAP[7]),
    ]
    try:
        font_size = max(16, height // 12)
...
def grid_to_rgb(grid: np.ndarray, scale: int = 1) -> np.ndarray:
    ...
    tls_positions = np.argwhere(grid == 7)
    if tls_positions.size:
        radius = max(3, int(scale * 1.5))
        for y, x in tls_positions:
            if scale > 1:
                y0 = y * scale
                x0 = x * scale
            else:
            ...
            y_end = min(rgb.shape[0], y0 + radius)
            x_start = max(0, x0 - radius)
            x_end = min(rgb.shape[1], x0 + radius)
            rgb[y_start:y_end, x_start:x_end] = COLOR_MAP[7]
    return rgb
