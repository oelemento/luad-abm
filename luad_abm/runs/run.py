"""Command-line interface to run LUAD Mesa simulations."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LUAD immune-stromal simulator")
    parser.add_argument("--preset", required=True, help="Path to preset JSON (e.g. config/G3_inflammatory.json)")
    ...
    parser.add_argument("--no-legend", action="store_true", help="Omit the color legend from the movie")
    parser.add_argument("--movie-scale", type=int, default=6, help="Integer upscaling factor applied to movie frames")
...
def main() -> None:
    ...
    viz.plot_state_metrics(timeseries_df, summary_dir)
    ...
    viz.plot_tls_quality(timeseries_df, summary_dir)
